...
(define the-grammar
  '((program (expression) a-program)
    ...
    ;array operations
    (expression
     ("newarray" "(" (arbno expression) ")") newarray-exp)
...
(define (value-of exp env)
  (cases expression exp
    ...
    (arrayref-exp (array place)
                  (let ((pl (value-of place env))
                        (ar (value-of array env)))
                    (let ((num (expval->num pl))
                          (vec (expval->array ar)))
                      (deref (vector-ref vec num)))))

    (arrayset-exp (array place newval)
                  (let ((ar (value-of array env))
                        (pl (value-of place env))
                        (nval (value-of newval env)))
                    (let ((vec (expval->array ar))
                          (num (expval->num pl)))
                          (begin
                            (vector-set! vec num nval)
                            (num-val 84))))) ;; a don't care value.
    ))
